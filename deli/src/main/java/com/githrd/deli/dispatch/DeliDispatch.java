package com.githrd.deli.dispatch;
...
/**
 * 	이 클래스는 .dlv라는 확장자로 요청이 된 경우 
 * 	요청 내용을 분석하고 실행할 클래스를 선택해서 실행(디스패치)시켜줄 서블릿클래스이다.
 * @author	안은비
 * @since	2022/05/23
 * @version	v.1.0
 * @see
 * 			com.githrd.deli.controller.DeliInter
 * 			com.githrd.deli.resources.dl.properties
 * 
 * 			작업이력 ]
 * 				2022.05.23	-	클래스제작
 * 								담당자 : 안은비
...
@WebServlet("*.dlv")
public class DeliDispatch extends HttpServlet {

	private HashMap<String, DeliInter> map; // 실제요청내용과 실제 실행할 객체를 입력할 맵
	
	public void init(ServletConfig config) throws ServletException {
		/*
			할일 ]
				최초로 이 서블릿이 시작되면(.dlv 로 처음 요청이 올때...)
		...
		try {
			...
			path = path + "../resources/dl.properties";
//			System.out.println("### dispatch path : " + path);
		...
		map = new HashMap<String, DeliInter>();
		// 키값만 뽑아오고
...
//		System.out.println("# service domain : " + domain);
		// 3. 실제요청을 알아내고
 		...
 		DeliInter deli = map.get(real);
 		
 		// 응답문서 인코딩
 		resp.setCharacterEncoding("UTF-8");
 		
 		// 실행하고
 		String view = deli.exec(req, resp);
